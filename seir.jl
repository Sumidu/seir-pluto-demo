...
md"""
# Creating a simple SEIR Model in Julia. I love/hate Julia.
...
md"""
We need several libraries to solve the ordinary differential equations described below.
...
md"""
The following block defines the set or differential equations typically used in a SEIR model.
To simplify the model we assume a population size of 1. This means that we calculate using percentage values and not number of people.

The model has several parameters:

* The $$R_0$$ defines the R value of the disease.
* The $$\gamma$$ defines the rate of infection.
*  $$\sigma$$ defines the rate of transfer between E-I akin to the incubation period.
*  $$\omega$$ defines the rate of waning immunity in this model.
"""
...
First, we split the state of the model (x) into its components s,e,i,r. 

We then apply the canges to the model as a vector using the differential equations.

$$\frac{dS}{dt} = -\gamma * R_0 * S * I + \omega * R$$

The negative term are the new infections driven by $$R_0$$ and $$\gamma$$. They depend on the amount of still susceptible (S) and already infected (I). We will see this term added in the following equation.
The second term re-adds the waned immunity people back from the recovered compartment (R)

$$\frac{dE}{dt} = + \gamma * R_0 * S * I - \sigma * E$$

Here, we also remove the ones that are beyond incubation period, depending on the parameter $$\sigma$$ and the amount of people exposed.

$$\frac{dI}{dt} = + \sigma * E - \gamma * I$$

We add, the ones that are beyond incubation period and remove the ones recovered according the the recovery parameter $$\gamma$$ and the amount of people infected.

$$\frac{dR}{dt} = +\gamma * I - \omega * R$$

Lastly, we add the recovered into the recovered compartment and remove the ones whose immunity has waned according the parameter $$\omega$$ and the amount of people recovered.
...
md"""
## ODE as a function
Our function is an ordinary differential equation that calculated the change between state x and a one step increase in time t. By changing the parameters, we can model different diseases and endemic states.

### Initial conditions and problem definition.
---
The next step is to define the initial condition. For this purpose we define an intial state x_0.
x_0 is a vector and consists of intial values for S, E, I, and R.
We first define an initially exposed population of 1E-7, as in a millionth of the population. Given a popoulation of 80 Mio. We assume 80 initially infected.
We also assume 0 people are already infections and we calculate the S value by removing the I and E values. We assume no one is already immune (r_0 = 0).

To solve an ordinary differential equation, we also need to define a timespan (_tspan_) over which the ODE should be applied. Here, _tspan_ is a tuple that defines start and endpoint. Here, 0 and dur days. The parameter dur is later bound to a PlutoUI Slider, so we can change it interactively.

Lastly, we define the ODE Problem using the ODEProblem function that requires the ODE Function
"""

# ╔═╡ 1a584fe8-6e2d-4a6a-bbee-61d8dc7ee57d
md"""
Running a solver over a problem will identify interpolation steps that are suitable to generate values that match the constraints of the ODE Problem and the initial conditions. 

The Tsit5 ODE solver is a 4th order Range Kutta interpolator by Tsitouras. This is a good selection for _non-stiff_ ODEs. If you are unsure which one to use. You can let Julia determine the "best" fit. For this you will need to install the DifferentialEquations package (a lot larger than OrdinaryDiffEq). This has an autodetect feature to pick the right solver.

More info here: [https://diffeq.sciml.ai/stable/solvers/ode_solve/](https://diffeq.sciml.ai/stable/solvers/ode_solve/) 
"""

# ╔═╡ 0352422e-f56c-46c5-b31f-91c1cc13a1f9
md"""
# Experimenting with different $$R_0$$ values. 

The benefit of Pluto.jl is that we can bind certain values to an interactive Slider.
Here we will bind the _rnaught_ variable to a slider.

rnaught is: $(@bind rnaught Slider(1:0.1:42, default = 3.5, show_value = true))

We also set the duration of the required solution using the duration variable. This variable is set here: $(@bind dur Slider(10:600, default=365, show_value= true)) days.

Lastly, we generated a typical SEIR plot using the Plots package (PyPlot under the hood).
"""


# ╔═╡ 7dbc78ae-35da-4fa1-bc5e-8c9ba4fd9de1
begin
function simple_SEIR(x, p, t; γ = 1/18, R₀ = rnaught, σ = 1/3.2, ω = 0.01)
    s, e, i, r = x
...
begin
	...
	tspan = (0.0, dur)  # ≈ 350 days
	prob = ODEProblem(simple_SEIR, x_0, tspan)
	
end

# ╔═╡ 73761b21-bf9c-48de-bf9f-489ceeea3a7e
sol = solve(prob, Tsit5())


# ╔═╡ 39d85f2a-9512-4f6e-8122-128fc111d04e
...
julia_version = "1.9.1"
manifest_format = "2.0"
project_hash = "ef4613426a7ff2ce508060f222029a2059a26b7b"
